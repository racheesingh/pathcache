...
hijack_numbers = {}
for fi in files:
    if "interception_potential" in fi:
        with open(fi) as fd:
            hijack_data = json.load(fd)
            for asn, percent in hijack_data.iteritems():
                int_percent = int(percent[1])/float(percent[2])
                if asn in hijack_numbers:
                    hijack_numbers[asn].append(int_percent)
                else:
                    hijack_numbers[asn] = [int_percent]
num_bins = 20
fig, ax = plt.subplots()
...
plt.savefig("interception_potential.pdf", bbox_inches='tight')
pdb.set_trace()
